...
async function getPRs(username, callback) {
  try {
    ...
    if (t === 0) {
      ...
      callback(ar_PR, data1.avatar_url, true) // valid username but not participated in Hacktoberfest
    }
    else {
      for (var i = 0; i < t; i++) {
      ...
      callback(ar_PR, item[0].user.avatar_url, false) // valid username & participated in hacktoberfest
    }
  } catch (error) {
    console.log('API error: ' + error)
    callback([], null, true) // invalid username
  }
}
